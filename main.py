...
# def initialize():
#     """Initialize the AI system (for Streamlit caching)"""
#     global app
#     # All initialization already runs at import, just return app
#     return app
...
# Load & split
docs = WebBaseLoader("https://en.wikipedia.org/wiki/Artificial_intelligence").load()
docs = RecursiveCharacterTextSplitter(chunk_size=500).split_documents(docs)

# Vector store
embeddings = HuggingFaceEmbeddings(model_name='all-MiniLM-L6-v2')
vectorstore = Cassandra(embedding=embeddings, table_name='qa_mini_demo')
...
route_prompt = ChatPromptTemplate.from_messages([
    ("system", "Route to vectorstore for AI/agents topics, else wiki."),
    ("human", "{question}")
])
...
def route_question(state):
    return "wiki_search" if question_router.invoke({"question": state["question"]}).datasource == "wiki_search" else "retrieve"

# Build graph
workflow = StateGraph(GraphState)
workflow.add_node("retrieve", retrieve)
workflow.add_node("wiki_search", wiki_search)
workflow.add_conditional_edges(START, route_question, {"retrieve": "retrieve", "wiki_search": "wiki_search"})
workflow.add_edge("retrieve", END)
workflow.add_edge("wiki_search", END)
